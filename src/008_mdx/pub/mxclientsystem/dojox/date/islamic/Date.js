//>>built
define("dojox/date/islamic/Date", ["dojo/_base/lang", "dojo/_base/declare", "dojo/date"], function(k, h, f) {
    var g = h("dojox.date.islamic.Date", null, {
        _date: 0,
        _month: 0,
        _year: 0,
        _hours: 0,
        ...
        constructor: function() {
            ...
            a ? 1 == a ? (a = arguments[0], "number" == typeof a && (a = new Date(a)), a instanceof Date ? this.fromGregorian(a) : "" == a ? this._date = new Date("") : (this._year = a._year, this._month = a._month, this._date = a._date,
        ...
        setDate: function(a) {
            ...
            if (!(0 < a && a <= this.getDaysInIslamicMonth(this._month, this._year))) {
                ...
                if (0 < a)
                    ...
                    for (b = this.getDaysInIslamicMonth(0 <=
                            this._month - 1 ? this._month - 1 : 11, 0 <= this._month - 1 ? this._year : this._year - 1); 0 >= a; b = this.getDaysInIslamicMonth(0 <= this._month - 1 ? this._month - 1 : 11, 0 <= this._month - 1 ? this._year : this._year - 1)) this._month--, 0 > this._month && (this._year--, this._month += 12), a += b
            }
            this._date = a;
            return this
        },
        ...
        setHours: function() {
            ...
            for (; 24 <= b;) this._date++, a = this.getDaysInIslamicMonth(this._month, this._year), this._date > a && (this._month++, 12 <= this._month && (this._year++, this._month -= 12), this._date -= a), b -= 24;
            this._hours = b
        },
        ...
        toGregorian: function() {
            var a = this._year,
                a = Math.floor(this._date + Math.ceil(29.5 * this._month) + 354 * (a - 1) + Math.floor((3 + 11 * a) / 30) + this._ISLAMIC_EPOCH - 1 - .5) + .5,
                b = a - this._GREGORIAN_EPOCH,
            ...
            4 != b && 4 != e && c++;
            b = a - (this._GREGORIAN_EPOCH +
                365 * (c - 1) + Math.floor((c - 1) / 4) - Math.floor((c - 1) / 100) + Math.floor((c - 1) / 400));
            d = this._GREGORIAN_EPOCH - 1 + 365 * (c - 1) + Math.floor((c - 1) / 4) - Math.floor((c - 1) / 100) + Math.floor((c - 1) / 400) + Math.floor(739 / 12 + (f.isLeapYear(new Date(c, 3, 1)) ? -1 : -2) + 1);
            d = a < d ? 0 : f.isLeapYear(new Date(c, 3, 1)) ? 1 : 2;
            b = Math.floor((12 * (b + d) + 373) / 367);
            d = this._GREGORIAN_EPOCH - 1 + 365 * (c - 1) + Math.floor((c - 1) / 4) - Math.floor((c - 1) / 100) + Math.floor((c - 1) / 400) + Math.floor((367 * b - 362) / 12 + (2 >= b ? 0 : f.isLeapYear(new Date(c, b - 1, 1)) ? -1 : -2) + 1);
            return new Date(c,
        ...
        fromGregorian: function(a) {
            ...
            var b = a.getFullYear(),
                c = a.getMonth(),
                d = a.getDate(),
                b = this._GREGORIAN_EPOCH - 1 + 365 * (b - 1) + Math.floor((b - 1) / 4) + -Math.floor((b - 1) / 100) + Math.floor((b - 1) / 400) + Math.floor((367 * (c + 1) - 362) / 12 + (2 >= c + 1 ? 0 : f.isLeapYear(a) ? -1 : -2) + d),
                b = Math.floor(b) + .5,
                b = b - this._ISLAMIC_EPOCH,
                c = Math.floor((30 * b + 10646) / 10631),
                d = Math.ceil((b - 29 - this._yearStart(c)) / 29.5),
                d = Math.min(d, 11);
            this._date = Math.ceil(b - this._monthStart(c,
                d)) + 1;
            this._month = d;
            this._year = c;
