...
define("dojo/data/ItemFileWriteStore", "../_base/lang ../_base/declare ../_base/array ../_base/json ../_base/kernel ./ItemFileReadStore ../date/stamp".split(" "), function(k, q, m, r, p, t, u) {
    return q("dojo.data.ItemFileWriteStore", t, {
        ...
        newItem: function(a, b) {
            ...
            if ("object" != typeof a && "undefined" != typeof a) throw Error("newItem() was passed something other than an object");
            var c =
                null,
            ...
            if (e === Number) c = this._arrayOfAllItems.length;
                ...
                if (k.isArray(c)) throw Error("newItem() was not passed an single-valued identity");
            }
            ...
            if (b && b.parent && b.attribute) {
                var e = {
                    ...
                    f = this.getValues(b.parent, b.attribute);
                if (f && 0 < f.length) {
                ...
                } else this._setValueOrValues(b.parent,
                    b.attribute, d, !1), e.newValue = d
            } else d[this._rootItemPropName] = !0, this._arrayOfTopLevelItems.push(d);
            ...
            for (var g in a) {
                ...
                if (this.referenceIntegrity)
                    for (f = 0; f < c.length; f++) h = c[f], this.isItem(h) && this._addReferenceToMap(h, d, g)
            }
            this.onNew(d, e);
            return d
        },
            ...
            if (this.referenceIntegrity) {
                ...
                m.forEach(e, function(b) {
                    m.forEach(this.getValues(a, b), function(c) {
                        this.isItem(c) && (a["backupRefs_" + this._reverseRefMap] || (a["backupRefs_" + this._reverseRefMap] = []), a["backupRefs_" + this._reverseRefMap].push({
                ...
                if (e = a[this._reverseRefMap])
                    for (var d in e) {
                        ...
                        if (f = this._itemsByIdentity ? this._itemsByIdentity[d] : this._arrayOfAllItems[d])
                            for (var h in e[d]) {
                                var g = this.getValues(f, h) || [],
                                    l = m.filter(g, function(a) {
        ...
        _setValueOrValues: function(a, b, c, e) {
            ...
            if (b == d) throw Error("ItemFileWriteStore does not have support for changing the value of an item's identifier.");
            var d = this._getValueOrValues(a, b),
                f = this.getIdentity(a);
            if (!this._pending._modifiedItems[f]) {
                var h = {},
                    g;
                for (g in a) h[g] = g === this._storeRefPropName || g === this._itemNumPropName || g === this._rootItemPropName ? a[g] : g === this._reverseRefMap ? k.clone(a[g]) : a[g].slice(0, a[g].length);
                this._pending._modifiedItems[f] = h
            }
            f = !1;
            if (k.isArray(c) && 0 === c.length) {
                if (f = delete a[b],
                    c = void 0, this.referenceIntegrity && d) {
                    g = d;
                    k.isArray(g) || (g = [g]);
                    for (var l = 0; l < g.length; l++) h = g[l], this.isItem(h) && this._removeReferenceFromMap(h, a, b)
                }
            } else {
                ...
                if (this.referenceIntegrity)
                    if (d) {
                        ...
                        for (l in n) this._removeReferenceFromMap(this._itemsByIdentity ?
                            this._itemsByIdentity[l] : this._arrayOfAllItems[l], a, b)
                    } else
                        for (l = 0; l < f.length; l++) h = f[l], this.isItem(h) && this._addReferenceToMap(h, a, b);
                a[b] = f;
                f = !0
            }
            if (e) this.onSet(a, b, d, c);
            return f
        },
        ...
        _removeReferenceFromMap: function(a, b, c) {
            ...
            var e = a[this._reverseRefMap],
                d;
            if (e) {
                for (d in e) d == b && (delete e[d][c], this._isEmpty(e[d]) && delete e[d]);
                this._isEmpty(e) &&
                    delete a[this._reverseRefMap]
            }
        },
                ...
                for (var b in this._datatypeMap) {
                    ...
                    if (k.isObject(c) && !k.isFunction(c)) {
                        if (a instanceof c.type) {
                            if (!c.serialize) throw Error("ItemFileWriteStore:  No serializer defined for type mapping: [" +
                                b + "]");
                            return {
                                _type: b,
        ...
        _getNewFileContentString: function() {
            var a = {},
                b = this._getIdentifierAttribute();
                ...
                if (null !== c) {
                    ...
                    for (d in c)
                        if (d !== this._storeRefPropName && d !== this._itemNumPropName && d !== this._reverseRefMap && d !== this._rootItemPropName) {
                            var f =
                                this.getValues(c, d);
                            if (1 == f.length) e[d] = this._flatten(f[0]);
                            else
                                for (var h = [], g = 0; g < f.length; ++g) h.push(this._flatten(f[g])), e[d] = h
                        } a.items.push(e)
                }
            }
            return r.toJson(a, !0)
        ...
        _isEmpty: function(a) {
            ...
            if (k.isObject(a))
                for (var c in a) {
                    b = !1;
                    break
                } else k.isArray(a) && 0 < a.length && (b = !1);
            return b
        },
        save: function(a) {
            this._assert(!this._saveInProgress);
            ...
            var b = this,
                c = function() {
                    ...
                    b._saveInProgress = !1;
                    a && a.onComplete && a.onComplete.call(a.scope ||
                        p.global)
                },
                e = function(c) {
                    b._saveInProgress = !1;
        ...
        revert: function() {
            ...
            for (var a in this._pending._modifiedItems) {
                var b = this._pending._modifiedItems[a],
                    c = null,
                    c = this._itemsByIdentity ? this._itemsByIdentity[a] : this._arrayOfAllItems[a];
                b[this._storeRefPropName] =
                    this;
                for (var e in c) delete c[e];
                k.mixin(c, b)
            }
            var d;
            for (a in this._pending._deletedItems) d = this._pending._deletedItems[a], d[this._storeRefPropName] = this, b = d[this._itemNumPropName], d["backup_" + this._reverseRefMap] && (d[this._reverseRefMap] = d["backup_" + this._reverseRefMap], delete d["backup_" + this._reverseRefMap]), this._arrayOfAllItems[b] = d, this._itemsByIdentity && (this._itemsByIdentity[a] = d), d[this._rootItemPropName] && this._arrayOfTopLevelItems.push(d);
            for (a in this._pending._deletedItems) d = this._pending._deletedItems[a],
                d["backupRefs_" + this._reverseRefMap] && (m.forEach(d["backupRefs_" + this._reverseRefMap], function(a) {
            ...
            for (a in this._pending._newItems) b = this._pending._newItems[a], b[this._storeRefPropName] = null, this._arrayOfAllItems[b[this._itemNumPropName]] = null, b[this._rootItemPropName] && this._removeArrayElement(this._arrayOfTopLevelItems, b), this._itemsByIdentity &&
                delete this._itemsByIdentity[a];
            this._pending = {
                _newItems: {},
                _modifiedItems: {},
                _deletedItems: {}
