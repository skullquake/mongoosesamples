...
define("dojo/request/iframe", "module require ./watch ./util ./handlers ../_base/lang ../io-query ../query ../has ../dom ../dom-construct ../_base/window ../NodeList-dom".split(" "), function(C, D, E, r, F, x, A, G, y, B, t, d) {
            ...
            if ("html" !== e) {
                if ("xml" === e)
                    if ("html" === g.documentElement.tagName.toLowerCase()) {
                        ...
                        var d = g.documentElement.innerText ||
                            g.documentElement.textContent,
                            d = d.replace(/>\s+</g, "\x3e\x3c");
                        a.text = x.trim(d)
                    } else a.data = g;
                else a.text = g.getElementsByTagName("textarea")[0].value;
                F(a)
            } else a.data = g
        } catch (h) {
            b = h
    ...
    function c(a, b, f) {
        ...
        a = d.url;
        b = d.options;
        if ("GET" !== b.method && "POST" !== b.method) throw Error(b.method + " not supported by dojo/request/iframe");
    ...
    c.create = function(a, b, c) {
        ...
        b = t.place('\x3ciframe id\x3d"' + a + '" name\x3d"' + a + '" src\x3d"' + c + '" onload\x3d"' + b + '" style\x3d"position: absolute; left: 1px; top: 1px; height: 1px; width: 1px; visibility: hidden"\x3e',
            d.body());
        return d.global[a] = b
    };
    c.doc = function(a) {
    ...
    c._fireNextRequest = function() {
        ...
        try {
            if (!c._currentDfd && c._dfdQueue.length) {
                do a = c._currentDfd = c._dfdQueue.shift(); while (a && (a.canceled || a.isCanceled && a.isCanceled()) && c._dfdQueue.length);
                if (!a || a.canceled || a.isCanceled && a.isCanceled()) c._currentDfd = null;
                else {
                    var b = a.response,
                        f = b.options,
                        g = a._contentToClean = [],
                        ...
                        h = f.data || null,
                        n;
                    a._legacy || "POST" !== f.method || e ? "GET" === f.method && e && -1 < b.url.indexOf("?") &&
                        (n = b.url.slice(b.url.indexOf("?") + 1), h = x.mixin(A.queryToObject(n), h)) : e = a._tmpForm = t.create("form", {
                            name: u + "_form",
                            style: {
                                position: "absolute",
                                top: "-1000px",
                    ...
                    if (e) {
                        if (!a._legacy) {
                            var k = e;
                            do k = k.parentNode; while (k && k !== d.doc.documentElement);
                        ...
                        if (h) {
                            ...
                            for (p in h) {
                                ...
                                if (x.isArray(q) && 1 < q.length)
                                    for (n = 0; n < q.length; n++) k(p, q[n]);
                                else e[p] ? e[p].value = q : k(p, q)
                            }
                        }
                        var v = e.getAttributeNode("action"),
                            m = e.getAttributeNode("method"),
                            w = e.getAttributeNode("target");
                        ...
                        w ? w.value = c._iframeName :
                            e.setAttribute("target", c._iframeName);
                        e.target = c._iframeName;
                        l && l.emit("send", b, a.promise.cancel);
                        c._notifyStart(b);
                        e.submit()
                    } else f = "", b.options.data && (f = b.options.data, "string" !== typeof f && (f = A.objectToQuery(f))), k = b.url + (-1 < b.url.indexOf("?") ? "\x26" : "?") + f, l && l.emit("send", b, a.promise.cancel), c._notifyStart(b), c.setSrc(c._frame, k, !0)
                }
            }
        } catch (M) {
            a.reject(M)
